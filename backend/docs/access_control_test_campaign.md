...
## My Decision Queue Source of Truth

My Decision Queue is generated by the backend endpoint `GET /decision-queue/my`. The frontend displays the returned committees and queue items and does not determine queue eligibility. Eligibility is based on the authenticated user's active committee membership, Authority Level, Board of Origin, risk workflow status, and fixed governance committee status for MIDDLE/HIGH queues.
