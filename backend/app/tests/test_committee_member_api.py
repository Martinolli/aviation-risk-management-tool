...
def test_committee_member_crud_duplicate_and_filters(client: TestClient, db_session: Session) -> None:
    ...
    create_response = client.post("/committee-members", json=payload)
    ...
    assert client.patch(f"/committee-members/{member_id}", json={"role_label": "Secretary"}).json()["role_label"] == "Secretary"
